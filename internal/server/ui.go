...
async function load(){var r=await fetch(A+'/scrape_jobs').then(function(r){return r.json()});items=r.scrape_jobs||[];document.getElementById('stats').innerHTML='<div class="st"><div class="st-v">'+items.length+'</div><div class="st-l">Total</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div><div class="st"><div class="st-v">-</div><div class="st-l">&nbsp;</div></div>';render();}
function render(){var q=(document.getElementById('search').value||'').toLowerCase();var f=items;
if(q)f=f.filter(function(i){return (i.name||'').toLowerCase().includes(q)||(i.url||'').toLowerCase().includes(q)||(i.selector||'').toLowerCase().includes(q)||(i.schedule||'').toLowerCase().includes(q)});
...
if(i.selector)h+='<span>'+esc(i.selector)+'</span>';
if(i.schedule)h+='<span>'+esc(i.schedule)+'</span>';
...
async function del(id){if(!confirm('Delete?'))return;await fetch(A+'/scrape_jobs/'+id,{method:'DELETE'});load();}
function formHTML(item){var i=item||{name:"",url:"",selector:"",schedule:"",last_result:"",status:"",run_count:""};var isEdit=!!item;
var h='<h2>'+(isEdit?'EDIT':'NEW')+' RUSTLER</h2>';
...
h+='<div class="fr"><label>Selector</label><input id="f-selector" value="'+esc(i.selector)+'"></div>';
h+='<div class="fr"><label>Schedule</label><input id="f-schedule" value="'+esc(i.schedule)+'"></div>';
...
async function submit(){var body={name:document.getElementById('f-name').value.trim(),url:document.getElementById('f-url').value.trim(),selector:document.getElementById('f-selector').value.trim(),schedule:document.getElementById('f-schedule').value.trim(),last_result:document.getElementById('f-last-result').value.trim(),status:document.getElementById('f-status').value.trim(),run_count:document.getElementById('f-run-count').value.trim()};
if(editId){await fetch(A+'/scrape_jobs/'+editId,{method:'PUT',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}
else{await fetch(A+'/scrape_jobs',{method:'POST',headers:{'Content-Type':'application/json'},body:JSON.stringify(body)});}closeModal();load();}
